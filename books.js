...
const borrowedBooksArr = [];

const addBooks = () => {
  booksDb.map((book, index) => {
    const dbBook = document.createElement("div");
    const dbBookTitle = document.createElement("h1");
    const dbBookAuthor = document.createElement("p");
    const dbBookLink = document.createElement("a");

    const btnContainer = document.createElement("div");
    const borrowBtn = document.createElement("button");
    const returnBtn = document.createElement("button");

    dbBook.classList = "dbBook";
    dbBook.setAttribute("id", index);

    dbBookTitle.classList = "dbBookTitle";
    dbBookAuthor.classList = "dbBookAuthor";
    dbBookLink.classList = "dbBookLink";

    btnContainer.classList = "btnContainerDiv";
    borrowBtn.classList = "borrowBtn btns";
    returnBtn.classList = "returnBtn btns";

    borrowBtn.addEventListener("click", () => {
      borrowedBooksArr.push(dbBook.id);
      localStorage.setItem("borrowedBooks", borrowedBooksArr);
      borrowBtn.disabled = true;
    });

    returnBtn.addEventListener("click", () => {
      let arr = localStorage.getItem("borrowedBooks");
      arr = arr.split(",");
      arr = arr.map((i) => Number(i));
      arr = arr.filter((el) => el != Number(dbBook.id));
      returnBtn.disabled = true;
      borrowBtn.disabled = false;
      localStorage.setItem("borrowedBooks", arr);
    });
    ...
    dbBookTitle.textContent = `Title: ${book.title}`;
    dbBookAuthor.textContent = `Author: ${book.author}`;
    dbBookLink.href = book.link;
    dbBookLink.target = "_blank";
    dbBookLink.innerText = `Link: ${book.link}`;

    borrowBtn.textContent = "Borrow";
    returnBtn.textContent = "Return";

    searchBook(book, dbBook);

    booksDiv.appendChild(dbBook);
    dbBook.appendChild(dbBookTitle);
    dbBook.appendChild(dbBookAuthor);
    dbBook.appendChild(dbBookLink);

    btnContainer.appendChild(borrowBtn);
    btnContainer.appendChild(returnBtn);
    dbBook.appendChild(btnContainer);
  });
};

const searchBook = (book, dbBook) => {
  searchBtn.addEventListener("click", () => {
    if (searchInput.value !== "") {
      if (
        searchInput.value.toLocaleLowerCase() !== book.title.toLocaleLowerCase()
      ) {
        dbBook.style.display = "none";
      } else {
        null;
      }
    } else {
      dbBook.style.display = "flex";
    }
  });
};
